-- SimpleFIN Budget App schema (Postgres / Supabase dialect).
-- Run this once in the Supabase SQL editor when setting up the cloud DB.
-- Types are kept TEXT where SQLite uses TEXT, so the runtime queries in
-- app/ingestion.py and app/budgets.py work unchanged on both backends.
...
CREATE TABLE IF NOT EXISTS transactions_raw (
    ...
    captured_at TEXT NOT NULL DEFAULT to_char(now(), 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    auto_category TEXT,
    first_seen_at TEXT NOT NULL DEFAULT to_char(now(), 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE OR REPLACE VIEW v_transactions_classified AS
SELECT
...
CREATE OR REPLACE VIEW v_account_effective_balance AS
SELECT
    a.id AS account_id,
    a.org_name,
    a.name,
